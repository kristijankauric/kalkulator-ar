...
## 14) Dnevnik promjena - placement i deploy (2026-03-31)
- Sto smo radili danas (redom):
  - `16f1ace`: vracen placement baseline iz `11:24` (`401792d`) za probu na mobitelu
  - `7e01532`: dodatni rollback na commit prije toga (`94e41ce`) jer je i `11:24` varijanta bila krivo pozicionirana
  - `751a2b0`: sync `docs/` builda za GitHub Pages preview test
- Trenutno aktivni mobile placement baseline:
  - `WorldTracker_6DOF.Place_6DOF` je na "pre-11:24" varijanti (commit `94e41ce`)
  - koristi indicator position pri kliku, uz original tracker start semantiku (`origPos` / postojeći `startZ`)
  - nema dodatnog "Strict Arrow Lock" clampa na `minZ`
- Trenutno aktivna deploy logika:
  - GitHub Pages workflow se automatski pokrece na branchu `main` i `dorade` SAMO ako push sadrzi promjene u `docs/**` ili `.github/workflows/deploy-pages.yml`
  - push koji mijenja samo `Assets/**` nece sam pokrenuti Pages deploy

## 15) Placement tracking WIP (2026-03-31, popodne)
- Status:
  - WIP, nije jos commitano na remote
- Sto je promijenjeno za bolji AR lock na plohu:
  - `Assets/Imagine/WorldTracker/Scripts/WorldTracker_6DOF.cs`
  - `Place_6DOF` sada koristi kameru u trenutku klika (`clickCamPos`) i racuna `startZ` iz udaljenosti kamera->indicator u trenutku place akcije
  - prije slanja `MODE=6DOF` radi se `SyncScreenSpacePosition()` da se smanji prvi-frame skok na stale target
- Sto je promijenjeno za "double arrow" vizual:
  - `Assets/Imagine/WorldTracker/Scripts/WorldTracker.cs`
  - u `Start()` i `ResetOrigin()` placement indicator ostaje skriven dok `UpdatePlacementIndicator()` ne postavi prvi validan polozaj
  - `Assets/Scenes/Digitron AR Base 3.unity`: child objekt `Placement Indicator/Shadow` je privremeno deaktiviran
- Napomena:
  - ove promjene trebaju mobilni test (place/reset/place i kretanje kamere) prije commita
